...
export async function getItems(bid, page, perPage, search, filterPriority) {

    let fetchStr = `${process.env.NEXT_PUBLIC_LOCALHOST_API_URL}/items?bag=${bid}&page=${page}&perPage=${perPage}`;
    if(search) fetchStr = fetchStr.concat('',`&search=${search}`);
    if(filterPriority) fetchStr = fetchStr.concat('',`&filterPriority=${filterPriority}`);
