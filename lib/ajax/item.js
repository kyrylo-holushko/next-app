...
export async function getItems(bid, page, perPage, search, filterPriority, order) {
    ...
    let fetchStr = `${process.env.NEXT_PUBLIC_API_URL}/items?bag=${bid}&page=${page}&perPage=${perPage}&column=${order.column}&order=${order.order}`;
    if(search) 
...
export async function editItem(form, iid) {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/items/${iid}`,{
        method: "PUT",
        body: formData,
        headers: {
            "authorization": `JWT ${getToken()}`
