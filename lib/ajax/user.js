...
export async function generateResetLink(email) {
	...
	const res = await fetch(`${process.env.NEXT_PUBLIC_LOCALHOST_API_URL}/user/password/link`, {
    	method: "POST",
...
export async function resetPassword(form, token) {
	...
	const res = await fetch(`${process.env.NEXT_PUBLIC_LOCALHOST_API_URL}/user/password/new`, {
    	method: "PUT",
    	body: JSON.stringify(form),
    	headers: {
